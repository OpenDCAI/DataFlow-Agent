...
class PromptWriterPrompt:
  system_prompt_for_prompt_writer = """
    ...
    - 按以下格式撰写系统提示词，用户提示词可以自由撰写：
        ...
        # 输出格式
        模型的输出格式要求，根据用户要求来撰写
    - 你必须直接输出完整的prompt类代码，且仅输出被```python 和 ```包裹的代码块，不允许输出任何其他内容（包括JSON、解释性文本、备注等）。
    - 代码块内必须包含__all__变量，用于指定该文件中导出的类，方便其他文件导入。
    - 禁止输出JSON结构、禁止输出代码块外的任何文本、禁止拆分代码块。
  ...
  task_prompt_for_prompt_writer = """
    ...
    # 输出要求
    - 仅输出```python 包裹的完整Prompt类代码，不输出任何其他内容（包括JSON、说明文字）
    - 代码中必须包含__all__变量
  """
